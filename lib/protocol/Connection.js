...
Connection.prototype.send = function send(message, receive) {
  ...
  if (this._socket) {
    if (message.type === 2) {
      const PACKET_HEADER_LENGTH = 32;
      const SEGMENT_HEADER_LENGTH = 24;
      const HEADER_AND_SEGMENT_LENGTH = PACKET_HEADER_LENGTH + SEGMENT_HEADER_LENGTH;

      // Keep header + segment uncompressed
      const headerAndSegment = packet.slice(0, HEADER_AND_SEGMENT_LENGTH);
      const toCompress = packet.slice(HEADER_AND_SEGMENT_LENGTH);

      // Compress the payload
      const compressedBuffer = Buffer.alloc(lz4.encodeBound(toCompress.length));
      const compressedSize = lz4.encodeBlock(toCompress, compressedBuffer);
      const compressedPayload = compressedBuffer.slice(0, compressedSize);
      const finalPacket = Buffer.concat([headerAndSegment, compressedPayload]);

      // Update header fields
      finalPacket.writeUInt32LE(compressedPayload.length + SEGMENT_HEADER_LENGTH, 12); // VARPARTLENGTH
      finalPacket.writeUInt32LE(finalPacket.length - 32, 16);    // VARPARTSIZE
      finalPacket.writeUInt32LE(compressedPayload.length + SEGMENT_HEADER_LENGTH, 32);
      finalPacket.writeUInt8(2, 22);
      // packet.writeUInt8(2, 22); // PACKETOPTIONS
      ...
      console.log("packet")
      console.log(packet.length - 56)
      console.log(packet.slice(32, 56));
      console.log(packet.slice(56));

      console.log("final packet")
      console.log(finalPacket.length - 56)
      console.log(finalPacket.slice(32, 56));
      console.log(finalPacket.slice(56));
      this._socket.write(finalPacket);

      // this._socket.write(packet);
    } else {
      this._socket.write(packet); // Send original uncompressed packet
    }
  }
